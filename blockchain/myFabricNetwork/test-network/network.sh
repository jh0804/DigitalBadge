...
function printHelp() {
  ...
  echo "    -ccep <policy>  - Optional, chaincode endorsement policy, using signature policy syntax. The default policy requires an endorsement from Sales and Customer"
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create Library Identities ######################"
    echo "##########################################################"

    set -x
    ../bin/cryptogen generate --config=./organizations/cryptogen/crypto-config-library.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    echo "##########################################################"
    echo "############ Create Student Identities ######################"
    echo "##########################################################"

    set -x
    ../bin/cryptogen generate --config=./organizations/cryptogen/crypto-config-student.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo $'\e[1;32m'"Failed to generate certificates..."$'\e[0m'
  ...
  echo
  echo "Generate CCP files for Library and Student"
...
function createConsortium() {
  ...
  set -x
  ../bin/configtxgen -profile OrdererGenesis -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
## call the script to join create the channel and join the peers of sales and customer
...
function networkDown() {
  # stop org3 containers also in addition to sales and customer, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/library/msp organizations/fabric-ca/library/tls-cert.pem organizations/fabric-ca/library/ca-cert.pem organizations/fabric-ca/library/IssuerPublicKey organizations/fabric-ca/library/IssuerRevocationPublicKey organizations/fabric-ca/library/fabric-ca-server.db'
#    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/sales2/msp organizations/fabric-ca/sales2/tls-cert.pem organizations/fabric-ca/sales2/ca-cert.pem organizations/fabric-ca/sales2/IssuerPublicKey organizations/fabric-ca/sales2/IssuerRevocationPublicKey organizations/fabric-ca/sales2/fabric-ca-server.db'
